<x-layout>
    <x-success></x-success>
    <x-back-link class="mb-1" href="{{ route('returns.items.index', [$store_return_id, $requisition_id]) }}">back</x-back-link>

    <!-- check if serial_numbers are present -->
    @if($serial_numbers->isEmpty())

        <p class="text-center fw-bold">{{ $item_name ?? 'Item' }} has no serial numbers</p>
                    ...
                    <div class="card-body bg-light bg-opacity-50">
                        <div class="table-responsive">
                            <table id="example1" class="table table-bordered table-striped table-hover"
                                data-title="{{ ucwords($item_name) }} Serial Numbers - {{ $requisition_id }}">
                                <thead>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($serial_numbers as $serial_number)
                                        <tr>
                                            ...
                                            <td class="fw-bold">{{ $serial_number->serial_numbers }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>

<x-layout>
    ...
    <div class="card p-5 bg-light bg-opacity-50">
        <div class="text-center">
            <h3 class="mb-2 text-bold">Edit Store Return</h3>
            <h5 class="mb-4">Store Requisition ID: {{ $requisitionId }}</h5>
            <h5 class="mb-4">Client Name:
                        @php
                            // Collect unique client-location pairs safely
                            
 $clients = $storeReturn->items
                ->map(fn($item) => optional(optional($item->destinationLink)->destination))
                ->filter() // remove nulls
                ->map(fn($dest) => ($dest->client ?? 'N/A') . ' - ' . ($dest->location ?? 'N/A'))
                ->unique()
                ->implode(', ');
        @endphp
        {{ $clients ?: 'N/A' }}
        ...
        <form method="post" action="{{ route('returns.update-all', $requisitionId) }}">
            @csrf
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="approved_by" name="approved_by" 
                       value="{{ old('approved_by', $storeReturn->approved_by) }}" required>
            </div>

            <div class="mb-3">
                <label for="return_date" class="form-label"><strong>Return Date</strong></label>
                <input type="date" class="form-control" id="return_date" name="return_date" 
                       value="{{ old('return_date', optional($storeReturn->returned_on)->format('Y-m-d')) }}" required max="{{ date('Y-m-d') }}">
            </div>

            <!-- Returned Items Section -->
<h4 class="mt-4 mb-3">Returned Items</h4>

@if($storeReturn->items->isNotEmpty())
        ...
        <tbody>
            @foreach($storeReturn->items as $item)
                ...
                <tr data-item-row data-item-name="{{ $item->item_name }}">
                    ...
                    <td>{{ $item->quantity }}</td>
                    <td>@php
        $requisitionItem = $storeReturn->recovery_store_requisition->items
...
<div class="text-center mt-4">
    ...
    <a href="{{ route('returns.index') }}" class="btn btn-secondary btn-lg">Cancel</a>
</div>

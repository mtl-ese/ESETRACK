<x-layout>
    ...
    <div class="card p-5 bg-light bg-opacity-50">
        <div class="text-center">
            <h3 class="mb-4">Emergency Return</h3>
        </div>

        <!-- Requisition Selection -->
        <div class="mb-4">
            <label for="requisition_select" class="form-label"><strong>Reference Emergency Requisition</strong></label>
            <div class="input-group">
                ...
                <datalist id="requisitions-list">
                    @foreach($requisitions as $requisition)
                        @php
                            $label = $requisition->department . ' (' . $requisition->initiator . ')';
                        @endphp
                        <option value="{{ $requisition->requisition_id }}" label="{{ $label }}"></option>
                    @endforeach
                </datalist>
        ...
        @if(!$hasRequisitions)
            <div class="mt-2 alert alert-info">There are no emergency requisitions yet.</div>
        @endif

        <!-- Materials Table (Hidden initially) -->
        <div id="materials_section" style="display: none;">
            <form method="post" action="{{ route('emergency.return.store') }}" id="return-form">
                @csrf
    ...
    <script>
        ...
        document.getElementById('load_materials').addEventListener('click', function() {
            ...
            if (!requisitionId) {
                if (datalist && datalist.options.length === 0) {
                    alert('There are no emergency requisitions yet');
                    return;
                }
        ...
        function loadMaterialsForRequisition(requisitionId) {
            ...
            const url = "{{ route('emergency.return.loadMaterials', ':requisitionId') }}".replace(':requisitionId', requisitionId);
            fetch(url)
        ...
        function renderMaterialsTable() {
            ...
            if (filteredMaterials.length === 0) {
                tbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted">All materials are fully requested or have zero balance.</td></tr>`;
                return;
            }
            ...
            filteredMaterials.forEach(material => {
                ...
                const requested = material.quantity ?? 0;
                const balance = material.balance ?? Math.max(requested - returned, 0);
                if (balance <= 0) return;
        ...
        document.getElementById('return-form').addEventListener('submit', function(e) {
            // Validate that serial selections match quantities for items with serials
            let allValid = true;
            const quantityInputs = document.querySelectorAll('input[name^="quantities["]');
            const materialContainers = document.querySelectorAll('[data-serial-container]');
            
            materialContainers.forEach(container => {
                const materialKey = container.getAttribute('data-material-key');
                const quantityInput = document.querySelector(`input[name="quantities[${CSS.escape(materialKey)}]"]`);
                const checkboxes = container.querySelectorAll('.serial-checkbox:checked');
                const quantity = parseInt(quantityInput?.value || 0, 10);
                
                // If quantity > 0 and there are serials in the dropdown, verify serials are selected
                if (quantity > 0 && container.querySelector('.serial-checkbox')) {
                    if (checkboxes.length === 0) {
                        alert(`Please select ${quantity} serial number(s) for ${materialKey}`);
                        allValid = false;
                    } else if (checkboxes.length !== quantity) {
                        alert(`Selected ${checkboxes.length} serial(s) but entered quantity is ${quantity} for ${materialKey}`);
                        allValid = false;
                    }
                }
            });

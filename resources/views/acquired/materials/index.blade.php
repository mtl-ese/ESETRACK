<x-layout>
    <x-success></x-success>
    <div class="d-flex">
        <x-back-link style="max-height: fit-content;" href="{{ route('acquired.index') }}">back</x-back-link>
    </div>
        ...
        <div class="col-12">
            <div class="card bg-light bg-opacity-50">
                <div class="card-header border-bottom-1">
                    <h3 class="card-title mt-2 text-lg"><strong>Acquired Materials</strong></h3>         
                </div>
                <div class="card-body bg-light bg-opacity-50">
                    <div class="table-responsive">
                        <table id="example1" class="datatable table table-bordered table-striped table-hover"
                            data-title="acquired items">
                            <thead>
                                <tr>
                                    ...
                                    <th style="color: white; background-color: #001f3f ;">Acquisition Date</th>
                                </tr>
                            </thead>
                            <tbody>
                                @php
                                $number = 1
                                @endphp
                                @foreach ($acquiredItems as $item)
                                <tr>
                                    <td class="text-decoration-none fw-bold" style="color: #007bff;">{{ $number++ }}</td>
                                    <td class="fw-bold" style="color: #007bff;">{{ $item->acquired->purchase_requisition_id }}</td>
                                    <td class="fw-bold" style="color: #007bff;">{{ $item->item_description }}</td>
                                    <td class="fw-bold" style="color: #007bff;">{{ $item->acquired->requisition->project_description }}</td>
                                    <td class="fw-bold" style="color: #007bff;">{{ $item->quantity }}</td>
                                    <td class="fw-bold" style="color: #007bff;"> {{ $item->acquired->requisition->requested_on }} </td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
